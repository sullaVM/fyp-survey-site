const express = require("express");
const exphbs = require("express-handlebars");
const bodyParser = require("body-parser");
const nodemailer = require("nodemailer");
const fs = require("fs");
const shuffle = require("shuffle-array");
const apiPort = 8082;
const app = express();

app.use(express.static("static"));
app.engine("handlebars", exphbs());
app.set("view engine", "handlebars");

app.use(bodyParser.urlencoded({ extended: true }));

const levelPairs = [
  [1, 2],
  [1, 3],
  [1, 4],
  [2, 3],
  [2, 4],
  [3, 4],
];

const labelSet = [
  "Full Face (Front)",
  "Full Face (Side Profile)",
  "Eye Close Up (Front)",
  "Eyes, Nose and Cheeks (Front)",
  "Eyes, Nose and Cheeks (Side)",
  "Mouth and Jaw (Front)",
  "Nose Close Up (Side)",
  "Skin Close Up (Front)",
];

const index = (request, response) => {
  const masterList = [];

  labelSet.forEach((label, i) => {
    levelPairs.forEach((pair) => {
      const shuffledPair = shuffle(pair);
      masterList.push({
        linkA: `img/Level${shuffledPair[0]}/l${shuffledPair[0]}-${i}.png`, // right
        linkB: `img/Level${shuffledPair[1]}/l${shuffledPair[1]}-${i}.png`, // left
        name: `${i},${shuffledPair[0]},${shuffledPair[1]}`,
        label,
        rightQuality: "B",
        leftQuality: "A",
      });
    });
  });

  const shuffledMasterList = shuffle(masterList);

  // Divide list into 6 parts
  if (shuffledMasterList.length != labelSet.length * levelPairs.length) {
    response.sendStatus(500);
  }

  const isSubmit = (section) => {
    if (section == levelPairs.length) {
      return true;
    }
    return false;
  };

  const ifSection1 = (section, value1, value2) => {
    if (section == 1) {
      return value1;
    }
    return value2;
  };

  const sections = [];
  const sectionLen = shuffledMasterList.length / levelPairs.length;
  let levelNum = 1;
  for (let i = 0; i < shuffledMasterList.length; i = i + sectionLen) {
    sections.push({
      hideDevice: ifSection1(levelNum, "", "hidden"),
      prevSection: ifSection1(levelNum, "tutorial", levelNum - 1),
      curSection: levelNum,
      nextSection: levelNum + 1,
      submit: isSubmit(levelNum),
      items: shuffledMasterList.slice(i, i + sectionLen),
    });
    levelNum++;
  }

  response.render("random", {
    sections,
  });
};

const defined = (_request, response) => {
  const sectionOne = [];
  const sectionTwo = [];
  const sectionThree = [];
  const sectionFour = [];

  for (let i = 0; i < labelSet.length; i++) {
    sectionOne.push({
      link: `img/Level1/l1-${i}.png`,
      label: labelSet[i],
      name: `l1-${i}`,
      quality: "A",
    });

    sectionTwo.push({
      linkA: `img/Level2/l2-${i}.png`,
      linkB: `img/Level1/l1-${i}.png`,
      label: labelSet[i],
      name: `l2-${i}`,
      rightQuality: "B",
      leftQuality: "A",
    });

    sectionThree.push({
      linkA: `img/Level3/l3-${i}.png`,
      linkB: `img/Level2/l2-${i}.png`,
      label: labelSet[i],
      name: `l3-${i}`,
      rightQuality: "C",
      leftQuality: "B",
    });

    sectionFour.push({
      linkA: `img/Level4/l4-${i}.png`,
      linkB: `img/Level3/l3-${i}.png`,
      label: labelSet[i],
      name: `l4-${i}`,
      rightQuality: "D",
      leftQuality: "C",
    });
  }

  response.render("index", {
    sectionOne,
    sectionTwo,
    sectionThree,
    sectionFour,
  });
};

const submit = (request, response) => {
  try {
    const source = request.header("user-agent");
    const answer = request.body;
    const data = {
      source,
      answer,
    };

    fs.appendFile(
      "responses.json",
      `${JSON.stringify(data, null, 4)},`,
      (err) => {
        if (err) {
          throw err;
        }
      }
    );

    response.render("success");
  } catch (error) {
    console.log(error);
    response.status(500).send("Something went wrong :(");
  }
};

const sendMail = async (request, response) => {
  try {
    const transporter = nodemailer.createTransport({
      host: "smtp.gmail.com",
      port: 465,
      secure: false,
      auth: {
        type: "OAuth2",
        user: process.env.EMAIL,
        serviceClient: process.env.CLIENT_ID,
        privateKey: process.env.PKEY,
      },
    });

    transporter.sendMail(
      {
        from: process.env.EMAIL,
        to: "montess@tcd.ie",
        subject: "Survey Response!",
        text: "test",
      },
      (error, info) => {
        if (error) {
          console.log(error);
        } else {
          console.log("Email sent: ", info.response);
        }
      }
    );

    response.sendStatus(200);
  } catch (error) {
    console.log(error);
    response.sendStatus(200);
  }
};

const parseAnswers = () => {
  let rawdata = fs.readFileSync("backup.json");
  let data = JSON.parse(rawdata);

  const sortByLevel = new Array(4);

  for (let i = 0; i < sortByLevel.length; i++) {
    sortByLevel[i] = new Array(8);
  }

  for (let i = 0; i < sortByLevel.length; i++) {
    for (let j = 0; j < sortByLevel[i].length; j++) {
      sortByLevel[i][j] = [[], [], []];
    }
  }

  data.forEach((item) => {
    const answer = item.answer;

    Object.keys(answer).forEach((key) => {
      let array = [];
      if (key.length === 0) {
        array = new Array();
      } else {
        array = key.replace(/, +/g, ",").split(",").map(Number);
      }

      if (array.length === 3) {
        const label = array[0];
        const levelA = array[1] - 1;
        const levelB = array[2] - 1;

        if (!sortByLevel[levelB]) {
          sortByLevel[levelB] = [];
        }

        if (!sortByLevel[levelB][label][levelA]) {
          sortByLevel[levelB][label][levelA] = [];
        }

        let ans = parseInt(answer[key]);

        switch (ans) {
          case 5:
            ans = 1;
            break;
          case 6:
            ans = 2;
            break;
          case 7:
            ans = 3;
        }

        let oppAns = 0;
        if (ans !== 0) {
          oppAns = ans * -1;
        }

        sortByLevel[levelB][label][levelA].push(ans);

        if (!sortByLevel[levelA]) {
          sortByLevel[levelA] = [];
        }

        if (!sortByLevel[levelA][label][levelB]) {
          sortByLevel[levelA][label][levelB] = [];
        }

        sortByLevel[levelA][label][levelB].push(oppAns);
      }
    });
  });

  sortByLevel.forEach((level, i) => {
    level.forEach((label, j) => {
      label.forEach((levelB, k) => {

        const average = levelB.reduce((a, b) => a + b, 0) / levelB.length;

        console.log(
          `${i + 1} VS ${k + 1} with label ${labelSet[j]} = ${average}`
        );
      });
      console.log('');

    });
    console.log('');
    console.log('');

  });
};

app.get("/", index);
app.post("/submit", submit);
app.get("/send", sendMail);
app.get("/random", defined);

parseAnswers();

app.listen(apiPort, () => console.log(`Listening on port: ${apiPort}`));
